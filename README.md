...
一只**浮在 Android 桌面上的鲸鱼女仆**。蓝发、女仆装、鲸鱼尾巴，可以拖着走，摸一下头她会撒娇。

## 形象

来自一张三视图设计稿，构建时切成三份并做背景透明化处理：
...
背景透明用的是**从图像边界泛洪填充**——只有和边界连通的白色才算背景，
所以围裙、领口、长袜这些被角色包围的白色能完整保留。
...
|---|---|
| **拖动** | 把她挪到你喜欢的任何位置 |
| **点一下** | 播放**摸头动画**，并冒出对话气泡（随机台词） |
| **点开后** | 弹出菜单：说点什么 / 跳一下 / 转个身 / 退出 |
| **重启手机** | 自动回来（前提是已授权） |

**动画效果**：
- 待机时缓慢上下浮动（呼吸感）
- 摸头动画共 5 帧，取自 issue #4 提供的 GIF，每帧 50ms，播放 3 轮后回到静止姿态
